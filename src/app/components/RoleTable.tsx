...
export default function RoleTable({ roles, departments }: Props) {
  ...
  return (
    <div className='mt-10 px-4'>
      <div className='flex justify-between'>
        <span></span>
        <button onClick={() => setShow(!show)} className='bg-gray-600 hover:bg-gray-400 transition-colors ease-in-out duration-250 cursor-pointer p-4 rounded-2xl text-white'>Add Role</button>
      </div>
      <div className="w-full overflow-x-auto">
        ...
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 150, maxWidth: 1 }} aria-label="simple table">
            <TableHead>
              <TableRow>
                <TableCell className={fontStyle}>Role Name</TableCell>
      ...
      {
        show ? <AddRole departments={departments} setShow={setShow} /> : null
      }
